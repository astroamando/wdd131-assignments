const temples = [
  ...
  /* --- 3 TEMPLOS ADICIONAIS (OBRIGATÓRIO) --- */
  {
...
// Função Principal de Criação de Cards
function createTempleCard(filteredTemples) {
    const resGrid = document.querySelector("#res-grid");
    resGrid.innerHTML = ""; // Limpar grid existente

    filteredTemples.forEach(temple => {
        let card = document.createElement("figure"); // Semanticamente melhor que div
        ...
        img.setAttribute("alt", `${temple.templeName} Temple`); // Acessibilidade
        img.setAttribute("loading", "lazy"); // Performance obrigatória
        img.setAttribute("width", "400"); // Boas práticas para evitar Layout Shift
...
// Escuta de Eventos (Event Listeners)
...
// Função auxiliar para mudar o título e gerenciar a classe 'active'
